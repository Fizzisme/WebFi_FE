...
function Code({ className, code, children, ...props }: CodeProps) {
  return (
    <CodeProvider value={{ code }}>
      <div
        className={cn(
          'relative flex flex-col overflow-hidden border bg-[#fff] dark:bg-[#171717] rounded-lg shadow-sm',
          className,
        )}
        {...props}
      >
...
function CodeHeader({
  ...
  return (
    <div
      className={cn(
        'bg-[#fff] dark:bg-[#202020] shrink-0 gap-x-2 border-b border-border/75 dark:border-border/50 text-sm flex text-muted-foreground items-center px-4 w-full h-10',
        className,
      )}
      {...props}
    >
...
function CodeBlock({ cursor, className, ...props }: CodeBlockProps) {
  ...
  return (
    <CodeBlockPrimitive
      ...
      className={cn(
        'relative text-sm p-4 overflow-auto min-h-0 flex-1',
